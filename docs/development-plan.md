...
| P11 | 官方测试集全量兼容推进 | parser / evaluator / functions / docs | 滚动 | 待评估 | 🚧 推进中（877/1251 可比对用例通过） |
| 合计 | | | **26** | **66** | |

> 当前固定快照（2026-06-28，P11 Lambda 签名与范围修复阶段）：`moon test` 为 157/157 通过；`moon check`、`moon info` 通过；`moon fmt` 已执行。JSONata 官方可比对审计为 `eligible 1251 / pass 877 / fail 374 / skip 431`。

### 1.1 当前暂停边界

P11 已完成日期时间 picture 修复、Lambda 签名语法与范围表达式修复，后续继续优先处理官方失败数最高的 group：

| 排名 | 官方 group | 失败数 |
| --- | --- | --- |
| 1 | `function-tomillis` | 34 |
| 2 | `joins` | 28 |
| 3 | `function-formatInteger` | 26 |
| 4 | `function-parseInteger` | 24 |
| 5 | `parent-operator` | 20 |
| 6 | `function-formatNumber` | 16 |
| 7 | `sorting` | 16 |
| 8 | `function-fromMillis` | 15 |
| 9 | `object-constructor` | 14 |
| 10 | `flattening` | 13 |
